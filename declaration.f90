...
! complex
  complex (q), parameter :: zero =(0._q,0._q)
...
! integers 
  ...
  integer :: l, spin1,  kp, iband, tband, sband
  integer ::  ios, N_sampling_z 
  integer :: ix, iy, iz, ip, i1, i2, i3
  integer :: NKPTS, spin_factor, nat, nV, jv, ndiv, npts, oerror
! From VASP
      INTEGER, PARAMETER :: ICMPLX=16,MRECL=10000000
      INTEGER IDUM, I, J, ISP, ISPIN
      INTEGER NPL, IREC, i_s, i_t
...
! reals 
  real (q) :: g1, g2, phi,Zmax,Zsurf,Zmin,z
...
! reals 
  ...
  real (q), allocatable :: ga (:), gb (:), gx(:), gy(:), gz(:), cngx(:,:), cngy(:,:)
